...
def myelectroneutrality(target,
                        pore_area='pore.area',
                        throat_area='throat.area',
                        pore_diffusivity='pore.diffusivity',
                        throat_diffusivity='throat.diffusivity',
    ...
    # Check if a concetration field is defined
    try:
        c1 = phase[pore_concentration][cn[:, 0]]
        c2 = phase[pore_concentration][cn[:, 1]]
    except KeyError:
    ...
    try:
        ...
        Dt = phase.interpolate_data(
            propname=pore_diffusivity)[throats]
    try:
        D1 = phase[pore_diffusivity][cn[:, 0]]
        D2 = phase[pore_diffusivity][cn[:, 1]]
    except KeyError:
        D1 = phase.interpolate_data(
            propname=throat_diffusivity)[cn[:, 0]]
        D2 = phase.interpolate_data(
            propname=throat_diffusivity)[cn[:, 1]]

    g1[m1] += 2 * F**2 * (D1*A1*c1)[m1] / (R * T1 * L1[m1])
    g2[m2] += 2 * F**2 * (D2*A2*c2)[m1] / (R * T2 * L2[m2])
    gt[mt] += 2 * F**2 * (Dt*At*ct)[mt] / (R * Tt * Lt[mt])
    # Preallocating g_inv
    g_inv1, g_inv2, g_invt = _np.zeros((3, len(Lt)))
    f1, f2, ft = [gi != 0 for gi in [g1, g2, gt]]
    g_inv1[~f1] = g_inv2[~f2] = g_invt[~ft] = _np.inf
...
def myelectroneutrality_2D(target,
                           pore_diameter='pore.diameter',
                           throat_diameter='throat.diameter',
                           pore_diffusivity='pore.diffusivity',
                           throat_diffusivity='throat.diffusivity',
    ...
    """
    return myelectroneutrality(target=target,
                               pore_area=pore_diameter,
                               throat_area=throat_diameter,
                               pore_diffusivity=pore_diffusivity,
                               throat_diffusivity=throat_diffusivity,
